...
class RelDataLayer(caffe.Layer):
	...
	def _get_next_batch(self):
		...
		for id in ids:
			...
			ims.append(self._concat_im(self._getAppr(im, sample["rBBox"]), self._getAppr(im, sample["rBBox_2"])))
			mask1 = self._concat_mask(self._getDualMask(ih, iw, sample["aBBox"]), self._getDualMask(ih, iw, sample["aBBox_2"])) 
			mask2 = self._concat_mask(self._getDualMask(ih, iw, sample["bBBox"]), self._getDualMask(ih, iw, sample["bBBox_2"]))
	...
	def setup(self, bottom, top):
		...
		self._shuffle_inds()
		top[0].reshape(self._batch_size, 2 * self._nclass)
