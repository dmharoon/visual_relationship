...
class RelDataLayer(caffe.Layer):
	...
	def _get_next_batch(self):
		...
		for id in ids:
			...
			label = (sample["aLabel"] - 1) * 100 + (sample["bLabel"] - 1) * 100 + (sample["rLabel"] )
			labels.append(label)
		return {"im": np.array(ims), "posdata": np.array(poses), "labels": np.array(labels)}
	...
	def setup(self, bottom, top):
		...
		self._name_to_top_map = {"im": 0, "posdata": 1, "labels": 2}
		self._shuffle_inds()
		...
		top[1].reshape(self._batch_size, 2, 32, 32)
